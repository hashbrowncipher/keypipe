...
def _serialize_provider_header(name, blob):
    ...
    lengths = struct.pack(PROVIDER_HEADER_FORMAT, len(encoded_name), len(blob))
    return lengths + encoded_name + blob


def serialize(salt, checksum, provider_blobs):
    serialized_blobs = [
        _serialize_provider_header(name, blob)
        for (name, blob) in sorted(provider_blobs.items())
    ]
    num_blobs = len(serialized_blobs)
    packed = struct.pack(
        INITIAL_HEADER_FORMAT, MAGIC, VERSION, salt, checksum, num_blobs
    )
...
def _deserialize_all_provider_info(num_blobs, next_header, infile):
    for num_blobs in range(num_blobs, 0, -1):
        name_len, blob_len = struct.unpack(PROVIDER_HEADER_FORMAT, next_header)
